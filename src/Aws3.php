...
class Aws3
{
    ...
    {
        $this->urlHttp = $urls['http'];
        $this->urlHttps = $urls['https'];
    ...
    public function listFiles($bucket = false)
    {
        ...
        $xml_response = simplexml_load_string($this->response['message']);
    ...
    public function putObjectString($string, $uri, $requestHeaders = [])
    {
        if (extension_loaded('fileinfo')) {
            ...
            $requestHeaders['Content-Type'] = $fileInfo->buffer($string);
        }
    ...
    public function putObject($file, $uri, $requestHeaders = [])
    {
        ...
        $this->response = $this->send($file, $requestHeaders);
     ...
     * @return array
     */
    private function send($sourcefile = false, $headers = [])
    {
